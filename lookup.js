let manchuCore = require("./manchuCore"); 
//const openCC = require('opencc-js');
//const zhconverter = new OpenCC('t2s.json');
//openCC.Converter('hk', 'cn').then(convert => console.log(convert('漢字，簡體字')));
async function simplify(text){return text} //(text, callback) {var response = await openCC.Converter('hk', 'cn').then(convert => callback(convert(text)));return response;}
...
async function any(term, mode) {
  ...
  try {
    ...
    if (/[\u4e00-\u9fa5]+/.test(t)) {
      ...
      if (manchuCore.isManchuScript(t)) {
  ...
  if (l > pagelength) {
    ...
    numb += `Page ${page} / ${pagecount}, ${pagelength * (page - 1) + 1} ~ ${
      pagelength * page
    } <i>of</i> ${l} result`;
  }
  if (l > 1) numb += 's';
  ...
  if (!realRegex(t)) docso = docso.replaceAll(t, '<u>' + t + '</u>');
  //<<<<<<< patch-1
  o = o + docso;
  docso = undefined;
